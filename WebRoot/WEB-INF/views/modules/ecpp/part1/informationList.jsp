...
<body>
	...
	<form:form id="searchForm" modelAttribute="information" action="${ctx}/ecpp/information/list" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="information" varStatus="i">
			...
			<%-- <td class="alignCenter">
				...
				<c:choose>
				    <c:when test="${information.status == '2' || information.status == '3'}">
				       	<shiro:hasPermission name="ecpp:information:edit">
						<td class="alignCenter" style="text-align:center;">
		    				<a href="${ctx}/ecpp/information/tijiaolist?id=${information.id}">提交</a>
		    				<a href="${ctx}/ecpp/information/form?id=${information.id}">修改</a>
							<a href="${ctx}/ecpp/information/delete?id=${information.id}" onclick="return confirmx('确认要删除该信息吗？', this.href)">删除</a>
						</td>
						</shiro:hasPermission>
				    ...
				    <c:otherwise>
				        <td style="text-align:center;"><a href="${ctx}/ecpp/information/informationDetails?id=${information.id}">查看</a></td>
				    </c:otherwise>
				</c:choose>	
