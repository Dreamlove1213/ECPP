...
<body>
	<ul class="nav nav-tabs">
		...
		<li><a href="${ctx}/ecpp/planinformation/a4List2?unit.id=${planinformation.unit.id}&unit.type=${planinformation.unit.type}">目标列表</a></li>
	...
	<form:form id="inputForm" modelAttribute="planinformation" action="${ctx}/ecpp/planinformation/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				...
				<script type="text/template" id="improvementsTpl">//<!--
						...
						<tr>
							<td colspan="5"><font style="color:black;">改进项内容：</font>{{row.content}}</td>
						</tr>
						<tr>
							<td colspan="5"><p style="line-height:20px;">改进项成果：</p>
						...
						<tr>
							<td colspan="5"><p style="color:black;line-height:20px;">实施历史：</p>
						...
						<tr>
							<td colspan="5"><font style="color:black;">得分：</font>{{row.score}}</td>
						</tr>
						<c:if test="${ecppConfig.attribute1 ==1}">
						<tr>
							<td colspan="5"><font style="color:black;">1阶段措施实施情况小结：</font>{{row.attribute3}}</td>
						</tr>
						<tr>
							<td colspan="5"><font style="color:black;">1阶段提升效益：</font>{{row.money1}}</td>
						</tr>
						</c:if>
						<c:if test="${ecppConfig.attribute2 == 1}">
						<tr>
							<td colspan="5"><font style="color:black;">2阶段措施实施情况小结：</font>{{row.attribute4}}</td>
						</tr>
						<tr>
							<td colspan="5"><font style="color:black;">2阶段提升效益：</font>{{row.money2}}</td>
						</tr>
						</c:if>
						//-->
					</script>
		...
		<c:if test="${!empty resultrecord.opinion}">
		 ...
		 <c:if test="${!empty planinformation.selfevaluationscore}">
			<div class="control-group">
				<label class="control-label"><font style="color:black;">目标分：</font></label>
				<div class="controls">
					<p class="con">${planinformation.selfevaluationscore}</p>
				</div>
			</div>
		...
		<%--<div class="control-group">
			<label class="control-label"><font style="color:black;">自我评价：</font></label>
			<div class="file" id="fileBox"></div>
		</div>--%>
