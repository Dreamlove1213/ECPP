...
<body>
	...
	<form:form id="inputForm" modelAttribute="planinformation" action="${ctx}/ecpp/planinformation/save" method="post" class="form-horizontal">
		...
		<div class="form-actions">
			...
			<%--<c:choose>
				<c:when test="${planinformation.shstatus eq '0'}">
					<font style="color:black;">审核：</font>
					<button class="btn btn-primary" onclick="pass('${planinformation.id}')">通过</button>
					&nbsp;
					<button class="btn btn-danger" onclick="noPass('${planinformation.id}')">驳回</button>
				</c:when>
				<c:otherwise>
					<input id="btnCancel" class="btn" type="button" value="返 回" onclick="history.go(-1)"/>
				</c:otherwise>
