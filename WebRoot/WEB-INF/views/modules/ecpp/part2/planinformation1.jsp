...
<body>
	...
	<form:form id="inputForm" modelAttribute="planinformation" action="${ctx}/ecpp/planinformation/save" method="post" class="form-horizontal">
			...
			<div class="controls">
				...
				<script type="text/template" id="improvementsTpl">//<!--
						<tr id="improvementsList{{idx}}" class="backColor">
							...
							<td>
								改进项标题：{{row.improvedtitle}}
							</td>
							<td>
								权重：{{row.weight}}%
							</td>
							<td>
								进度：{{row.improvementprogress}}%
							</td>
							<td>
								责任人：{{row.principal}}
							</td>
						...
						<tr>
							<td colspan="6">改进项内容：{{row.content}}</td>
						</tr>
						<tr>
							<td colspan="6">定期记录改进措施实施情况：{{row.attribute2}}</td>
						</tr>
						<tr>
							<td colspan="6">1阶段措施实施情况小结：{{row.attribute3}}</td>
						</tr>
						<tr>
							<td colspan="6">2阶段措施实施情况小结：{{row.attribute4}}</td>
						</tr>
		...
		<c:if test="${!empty resultrecord.situationandresults}">
			<div class="control-group">
				<label class="control-label">提升效果及情况：</label>
				<div class="controls">
