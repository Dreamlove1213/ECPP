...
<body>
	...
	<form:form id="inputForm" modelAttribute="planinformation" action="${ctx}/ecpp/planinformation/save" method="post" class="form-horizontal">
		...
		<c:if test="${!empty resultrecord.opinion}">
			...
			<div class="control-group">
				<label class="control-label" style="width: 130px;"><font style="color:black;">改革小组意见：</font></label>
				<div class="controls">
					<c:forEach items="${resultrecords}" var="res" varStatus="i">
					<!-- <pre style="width:90%;"></pre> -->
					<p class="con">${i.index+1}.${res.opinion} ——<fmt:formatDate value="${res.opinionchangedate}" pattern="yyyy年 MM月 dd日  HH时:mm分"/></p>
					</c:forEach>
				</div>
			</div>
