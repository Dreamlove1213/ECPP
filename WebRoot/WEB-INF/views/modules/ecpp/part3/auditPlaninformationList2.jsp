...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/ecpp/planinformation/a3ListWorkPlan?unit.id=${officeId}&unit.type=${type}">工作计划列表</a></li>
		<li class="active"><a href="${ctx}/ecpp/planinformation/a3List2?unit.id=${officeId}&unit.type=${type}">目标列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="planinformation" action="${ctx}/ecpp/planinformation/a3List2" method="post" class="breadcrumb form-search">
		...
		<c:forEach items="${page.list}" var="planinformation" varStatus="i">
			<tr>
				...
				<shiro:hasPermission name="ecpp:planinformation:edit"><td class="alignCenter" style="white-space: nowrap;">
    				<a href="${ctx}/ecpp/planinformation/a3View2?id=${planinformation.id}">查看</a>&nbsp;
    				<%--<c:if test="${!empty planinformation.situationandeffect}">
    					<a href="${ctx}/ecpp/planinformation/a3Form1?id=${planinformation.id}">目标跟踪</a>
    				</c:if>--%>
