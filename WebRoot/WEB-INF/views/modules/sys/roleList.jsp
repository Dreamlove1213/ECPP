...
<body>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${list}" var="role">
			<tr>
				<td class="alignCenter"><a href="form?id=${role.id}">${role.name}</a></td>
				<td class="alignCenter"><a href="form?id=${role.id}">${role.enname}</a></td>
				<td class="alignCenter">${role.office.name}</td>
				<td class="alignCenter">${fns:getDictLabel(role.dataScope, 'sys_data_scope', '无')}</td>
				<shiro:hasPermission name="sys:role:edit"><td class="alignCenter">
