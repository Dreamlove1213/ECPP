...
var uploadTools = {
    ...
    "getInitOption":function(uploadId){
        ...
        var initOption={
            "uploadId":uploadId,
            "uploadUrl":"/a/ecpp/information/fileUpload",//必须，上传地址
...
//                alert("success");
    ...
    "initWithLayout":function(opt){
        ...
        if(!opt.isHiddenCleanBt){
            ...
            btsStr += "<i class='iconfont icon-qingchu' style='font-size:20px;'>清除</i>";
            btsStr += " </div>";
        }
        btsStr += "</div>";
     ...
     * 上传文件
     */
    "uploadFile":function(opt,id){
        uploadTools.startUpload(opt);
        var uploadUrl = opt.uploadUrl+"?id="+id;
        var fileList = uploadFileList.getFileList(opt);
        var rememberFile = [];
